...
// Version     : 1.0
// Description : CSV Parser Implementation File
//============================================================================
...
void Parser::parseContent(void) {
	...
	for (int idx = 0; it != _originalFile.end(); it++, idx++) {
		bool quoted = false;
		int tokenStart = 0;
		unsigned int i = 0;
		...
		if (row->size() < _header.size()) {
			std::cerr << row; 
			throw Error("corrupted data in row #" + std::to_string(idx + 1) +
			            ". Row doesn't have the same number of columns as header");
		}
